...
class form {
		...
		for ($carg=0;$carg<func_num_args();$carg=$carg+3) {
			...
			for($i=0;$i<count($this->data);$i++) {
				...
				if (isset($key[1]) && $key[1] != '') {
					if ($ckey == $key[0]) {
						$this->data[$i]['value'] = isset($db_data[0][$key[1]])?$db_data[0][$key[1]]:null;
					}
				}
			}
		}
	...
	function db_set($pdata=array()) { 
		...
		for ($carg=1;$carg<func_num_args() || $carg==1;$carg=$carg+3) {
			$ckey = isset($args[$carg])?$args[$carg]:null;
			unset($data);
			...
			$field = isset($args[$carg+1])?$args[$carg+1]:null;
			$value = isset($args[$carg+2])?$args[$carg+2]:null;
	...
	function db_data_remove() {
		...
		//cirrus was this needed?
		//$this->data = array_merge_check($this->data);
	}
