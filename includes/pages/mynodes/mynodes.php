 ...
 * Copyright (C) 2011-2012 Leonidas Papadopoulos <stargazer@wna.gr>
 *
...
class mynodes {
	...
	function has_owner_access() {
		...
		/*		$this_node_com_admins=$db->get('communities.admins', "communities,  LEFT JOIN nodes ON nodes.community_id = communities.id", "nodes.node_id = '".intval(get('node'))."'");
		print_r($this_node_com_admins);
				$this_node_com_admins = explode(",",this_node_com_admins);*/
	...
	function form_node() {
		...
		$form_node->db_data_enum('nodes.community_id', $db->get("id AS value, name AS output", "communities"));#@#*** // changes output
	...
	function table_links() {
		...
		#@# table diasyndeseis otan kaneis edit (nea sthlh ip address apo to links__ip
		$table_links->db_data(
			'links.id, links.type, "" AS peer, links.peer_node_id, n_p.name AS peer_node_name, n_c.name AS peer_ap_nodename, n_c.id AS peer_ap_nodeid, l_c.ssid AS peer_ap_ssid, links.ssid, links.status, links.ip',
			'links
		...
		#if ($key != 0) {
				#@#
		#		$table_links->data[$key]['links__ip'] = long2ip($table_links->data[$key]['links__ip']);#@#
		#	}
		#}
		...
		for($i=1;$i<count($table_links->data);$i++) {
			if (isset($table_links->data[$i])) {
			 $table_links->data[$i]['ip'] = long2ip($table_links->data[$i]['ip']); //fix-show ip correctly #@# stargazer all cases
	...
	function output() {
		...
		if (get('action') == 'delete') {
			if ($db->del('nodes', "id = ".intval(get('node')))) {
				log_admin_action($nodes,get('node'), "delete_success");#@#
	...
	function output_onpost_form_node() {
		...
		if ($ret && $main->userdata->privileges['admin'] === TRUE && get('node') != 'add' && get('node') != $_POST['nodes__id']) {
			...
			$db->set('nodes_services', array('node_id' => $_POST['nodes__id']), "node_id = ".intval(get('node')));
		...
		if ($ret) {
			log_admin_action($nodes,"node#".get('node_id'), (get('node') == 'add'?'insert':'edit').'_success');#@#
	...
	function output_onpost_table_ip_ranges() {
		...
		if ($ret) {
			log_admin_action($nodes,"#ip_ranges_node#".get('node'), "update_success");#@#
	...
	function output_onpost_table_dns() {
		...
		if ($ret) {
			log_admin_action($nodes,"#dns_zones_node#".get('node'), "update_success");#@#
			$main->message->set_fromlang('info', 'update_success', makelink("",TRUE));
		} else {
			$main->message->set_fromlang('error', 'generic');
		}
	...
	function output_onpost_table_nameservers() {
		...
		if ($ret) { 			
		    log_admin_action($nodes,"#dns_nameservers_node#".get('node'),get('dns_nameservers'), "update_success");#@#
			$main->message->set_fromlang('info', 'update_success', makelink("",TRUE));
		} else {
			$main->message->set_fromlang('error', 'generic');
		}
	...
	function output_onpost_table_links() {
		...
		if ($ret) {
			log_admin_action($nodes,"#links_node#".get('node'),get('links'), "links", "delete_success");#@#
	...
	function output_onpost_table_links_ap() {
		...
		if ($ret) {
			log_admin_action($nodes,"#links_ap_node#".get('node'),get('links'), "links_ap", "delete_success");#@#
			$main->message->set_fromlang('info', 'delete_success', makelink("",TRUE));
		} else {
			$main->message->set_fromlang('error', 'generic');
		}
	...
	function output_onpost_table_subnets() {
		...
		if ($ret) {
			log_admin_action($nodes,"#subnets_node#".get('node'),get('subnets'), "subnets", "delete_success");#@#
			$main->message->set_fromlang('info', 'delete_success', makelink("",TRUE));
		} else {
			$main->message->set_fromlang('error', 'generic');
		}
	...
	function output_onpost_table_ipaddr() {
		...
		if ($ret) {
			log_admin_action($nodes,"#ip_adrresses_node#".get('node'),get('ip_addresses'), "ip_addresses", "delete_success");#@#
			$main->message->set_fromlang('info', 'delete_success', makelink("",TRUE));
		} else {
			$main->message->set_fromlang('error', 'generic');
		}
	...
	function output_onpost_table_services() {
		...
		if ($ret) {
			log_admin_action($nodes,"#nodes_services_node#".get('node'), "delete_success");#@#
			$main->message->set_fromlang('info', 'delete_success', makelink("",TRUE));
		} else {
			$main->message->set_fromlang('error', 'generic');
		}
