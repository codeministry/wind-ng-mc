...
class hostmaster_dnsnameservers {
	...
	function table_nameservers() {
		...
		$table_nameservers->db_data(
			'dns_nameservers.id, dns_nameservers.name,communities.name AS communities__name, nodes.name_ns, dns_nameservers.ip, communities.dnstld , dns_nameservers.date_in, dns_nameservers.status, dns_nameservers.delete_req',
			'dns_nameservers
	...
	function output_onpost_table_nameservers() {
		...
		if ($ret) {
			log_admin_action("namservers#".get('dns_nameservers'),get('id'), 'delete_success');#@#
