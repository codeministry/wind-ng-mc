...
class hostmaster_dnsnameserver {
	...
	function table_dns() {
		...
		$table_dns->db_data(
			'dns_zones.id, dns_zones.name, dns_zones.date_in, dns_zones.status, dns_zones.delete_req, dns_zones.type, communities.dnstld',
			'dns_zones ' .
			'INNER JOIN dns_zones_nameservers ON dns_zones_nameservers.zone_id = dns_zones.id
			 ...
			 INNER JOIN communities ON nodes.community_id = communities.id ',
		...
		for($i=1;$i<count($table_dns->data);$i++) {
			if (isset($table_dns->data[$i])) {
				...
				if ($table_dns->data[$i]['type'] == 'forward') $table_dns->data[$i]['name'] .= ".".$table_dns->data[$i]['dnstld'];
	...
	function output() {
		...
		if(get('action') === "delete")
		{
			...
			if ($ret) {
				log_admin_action("dns_nameservers#".get('nameserver'), 'delete_success');#@#
