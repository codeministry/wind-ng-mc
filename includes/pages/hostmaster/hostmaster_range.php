...
class hostmaster_range {
	...
	function has_edit_access() { //added by stargazer
		...
		if ($main->userdata->privileges['cadmin']===TRUE) {
			...
			Inner Join `ip_ranges` ON `nodes`.`id` = `ip_ranges`.`node_id`
			WHERE
		...
		return TRUE;}
		if ($main->userdata->privileges['hostmaster']===TRUE) {
		return TRUE;}
	...
	function form_range() {
		...
		if (!$this->has_edit_access()){$main->message->set_fromlang('info', 'no_privilege');}
		$form_range = new form(array('FORM_NAME' => 'form_range'));
		$form_range->db_data('ip_ranges.ip_start, ip_ranges.ip_end, ip_ranges.info, ip_ranges.status, ip_ranges.node_id');
	...
	function table_node_info() {
		...
		$table_node_info->db_data(
			'nodes.name AS nodes__name, nodes.id, nodes.date_in, nodes.info, areas.name AS areas__name, regions.name AS regions__name, communities.name',
			'ip_ranges ' .
		...
		#$table_node_info->db_data_enum('communities.name', $db->get("name AS value, fullname AS output", "communities"));#@#
