...
class nodes_search {
	...
	function form_search_nodes() {
		...
		array_push($form_search_nodes->data, array('Compare' => 'numeric', 'Field' => 'freeifs', 'fullField' => 'freeifs'));
		#array_push($form_search_nodes->data, array('Field' => 'freeifs', 'fullField' => 'freeifd', 'Type' => 'enum', 'Type_Enums' => array(array('value' => 'Y', 'output' => $lang['yes']), array('value' => 'N', 'output' => $lang['no']))));
	...
	function table_nodes() {
		...
		$table_nodes->db_data(
        	...
        	COUNT(DISTINCT cl.id) AS total_active_clients,  nodes.freeifs AS nodes__freeifs',
			'nodes
