...
class nodes_view {
	...
	function table_links_p2p() {
		...
		$table_links->db_data(
			'"" AS distance, n1.name AS node_name, n1.id AS node_id, n2.name AS peer_node_name, l1.type AS links__type, l1.info AS links__info, l1.peer_node_id AS links__peer_node_id, l1.date_in AS links__date_in, l1.ssid AS links__ssid, l1.protocol AS links__protocol, l1.channel AS links__channel, l1.equipment AS links__equipment, l1.status AS l1_status, l2.status AS l2_status, l1.security AS l1_security, l2.security AS l2_security, "" AS links__security,  "" AS links__status, l1.ip AS links__ip', 
			'links AS l1
		...
		$table_links->db_data(
			'"" AS distance, n1.name AS node_name, n1.id AS node_id, n2.name AS peer_node_name, l1.type AS links__type, l1.info AS links__info, l2.node_id AS links__peer_node_id, l1.date_in AS links__date_in, l2.ssid AS links__ssid, l2.protocol AS links__protocol, l2.channel AS links__channel, l1.equipment AS links__equipment, l1.status AS l1_status, l2.status AS l2_status,l1.security AS l1_security, l2.security AS l2_security, "" AS links__security, "" AS links__status,"" AS eq, l1.ip AS links__ip', 
		...
		foreach( (array) $table_links->data as $key => $value) {
			...
			#@#
			if ($key != 0) {
				if ($table_links->data[$key]['l1_security'] == 'sec_active' && $table_links->data[$key]['l2_security'] == 'sec_active') {
					$table_links->data[$key]['links__security'] = 'sec_active';
				} else {
					$table_links->data[$key]['links__security'] = 'sec_inactive';
				}
				$table_links->info['EDIT'][$key] = makelink(array('page' => 'nodes', 'node' => $table_links->data[$key]['links__peer_node_id']));
			}
			#@#
	...
	function output() {
		...
		#@#>
		foreach( (array) $t as $key => $value) {
			if ($value['type'] == 'hs') $this->tpl['table_links_ap'][$value['id']] = $construct->table($this->table_links_ap($value['id']), __FILE__);
		}
		#@#<
		...
		if (($main->userdata->privileges['cadmin']) && ($main->userdata->privileges['cadmin'] === TRUE)) $this->tpl['edit_node'] = makelink(array("page" => "mynodes", "node" => get('node') ));
